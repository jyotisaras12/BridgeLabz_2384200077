...
class LargestDigitArray{
    static void Main(){
        ...
        while(number != 0){
			if (index == maxDigit){
                maxDigit += 10; //increasing the maxDigit by 10
                int[] temp = new int[maxDigit]; //creating a new temp array
                //copying the existing digits into the temp array
                Array.Copy(digits, temp, digits.Length);
                digits = temp;	//assigning temp to digits
            }
